...
//   // Show backend error toast only (like "User already exists")
//   useEffect(() => {
//     if (error) {
//       toast.error(error);
//     }
...
//     // Only validate frontend input silently (no toast)
...
//     } catch (err) {
//       console.error(err);
//       // Error toast already shown by useEffect via store error
//     }
//   };
...
const SignUpPage = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    const isMMMUTEmail = /^[a-zA-Z0-9._%+-]+@mmmut\.ac\.in$/.test(email);
    ...
    if (!isMMMUTEmail) {
      toast.error("Only MMMUT email addresses (@mmmut.ac.in) are allowed.");
      return;
    }
        ...
        <form onSubmit={handleSignUp} className="space-y-4">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              setEmail(val);
              setIsEmailValid(/^[a-zA-Z0-9._%+-]+@mmmut\.ac\.in$/.test(val));
            }}
            className={!isEmailValid ? "border-red-500 focus:ring-red-500" : ""}
          />
          {!isEmailValid && (
            <p className="text-sm text-red-400 -mt-2">Only @mmmut.ac.in emails are accepted.</p>
          )}
